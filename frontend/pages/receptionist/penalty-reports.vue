<template>
  <div class="min-h-screen bg-gradient-to-br from-[#f3f7fb] via-[#eef4fb] to-[#dbe6f3] text-slate-900">
    <nav class="sticky top-0 z-50 border-b border-white/70 bg-white/85 backdrop-blur-xl shadow-sm">
      <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
        ...
        <div class="flex flex-wrap items-center gap-2 rounded-full border border-slate-200/80 bg-white/80 p-2 text-sm font-medium text-slate-600 shadow-lg shadow-slate-200/40 backdrop-blur">
          ...
          <NuxtLink to="/receptionist/penalty-reports" class="rounded-full bg-slate-800 px-4 py-2 font-semibold text-white shadow-md">
            Laporan Denda
          </NuxtLink>
      ...
      <div class="relative mx-auto max-w-7xl px-4 py-14 sm:px-6">
        <div class="grid items-center gap-8 lg:grid-cols-[1.2fr_0.8fr]">
          <div>
            ...
            <h1 class="mt-4 text-4xl font-bold">Laporan Pembayaran Denda</h1>
            <p class="mt-4 max-w-2xl text-lg text-slate-100">Pantau transaksi denda checkout harian dan progres verifikasi pembayaran tamu.</p>
          </div>
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="rounded-3xl border border-white/15 bg-white/10 px-5 py-4 backdrop-blur">
              <p class="text-xs uppercase tracking-[0.2em] text-slate-200">Tanggal</p>
        ...
        <div class="mt-6 flex justify-end">
          <NuxtLink to="/receptionist/penalty-verifications" class="rounded-xl bg-slate-900/90 px-4 py-2 text-sm font-semibold text-white ring-1 ring-white/20 transition hover:bg-slate-900">
            Verifikasi Denda
          </NuxtLink>
        </div>
      </div>
    ...
    <section class="relative z-10 mx-auto -mt-12 max-w-7xl px-4 pb-12 sm:px-6">
      <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="grid gap-4 md:grid-cols-[1fr_auto]">
          <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700">Tanggal Laporan</label>
            <input v-model="selectedDate" type="date" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
          </div>
          <button @click="loadReport(1)" class="self-end rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Tampilkan</button>
        </div>
      </div>
      ...
      <div class="mb-6 grid gap-4 md:grid-cols-3">
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p class="mt-1 text-2xl font-bold text-slate-900">{{ summary.total_transactions || 0 }}</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p class="text-sm text-slate-500">Total Nominal</p>
          <p class="mt-1 text-2xl font-bold text-amber-700">Rp {{ formatPrice(summary.total_penalty_amount || 0) }}</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p class="text-sm text-slate-500">Nominal Terverifikasi</p>
          <p class="mt-1 text-2xl font-bold text-emerald-700">Rp {{ formatPrice(summary.verified_penalty_amount || 0) }}</p>
        </div>
      </div>
      ...
      <div class="overflow-hidden rounded-[2rem] border border-white/70 bg-white/92 shadow-2xl shadow-slate-200/60 backdrop-blur">
        <div class="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-zinc-100 px-8 py-6">
          <h2 class="text-2xl font-bold text-slate-900">Detail Transaksi Denda</h2>
        </div>
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-if="payments.length === 0">
                <td colspan="5" class="px-6 py-8 text-center text-slate-500">Belum ada pembayaran denda di tanggal ini.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const loadReport = async (page = 1) => {
  loading.value = true
  const result = await receptionistStore.getLateCheckoutPenaltyReport(selectedDate.value, authStore.token, page, perPage)
  summary.value = result.success ? (result.data?.summary || {}) : {}
  payments.value = result.success ? (result.data?.payments || []) : []
  pagination.value = result.success
