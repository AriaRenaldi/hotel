<template>
  ...
  <!-- Sidebar -->
  <aside :class="[mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0', collapsed ? 'lg:w-20' : 'lg:w-72']" 
         class="fixed left-0 top-0 h-full bg-gradient-to-b from-white via-sky-50 to-blue-50 text-gray-800 shadow-xl border-r border-blue-100 z-50 transition-all duration-300 ease-in-out flex flex-col">
    ...
    <div class="p-5 flex items-center justify-between border-b border-blue-100 min-h-[72px]">
      ...
      <button @click="toggle" class="hidden lg:flex p-1.5 rounded-lg hover:bg-sky-100 transition">
        <svg v-if="!collapsed" class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-1">
      ...
      <NuxtLink to="/admin/dashboard" :class="[active === 'dashboard' ? 'bg-gradient-to-r from-sky-100 to-blue-100 text-blue-700 shadow-sm' : 'text-gray-600 hover:bg-sky-50 hover:text-sky-700', collapsed ? 'justify-center px-3' : '']" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group relative" @click="closeMobile">
      ...
      <p v-if="!collapsed" class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 mb-2 mt-6">Manajemen</p>
      <NuxtLink v-for="item in manageItems" :key="item.to" :to="item.to" :class="[active === item.active ? 'bg-gradient-to-r from-sky-100 to-blue-100 text-blue-700 shadow-sm' : 'text-gray-600 hover:bg-sky-50 hover:text-sky-700', collapsed ? 'justify-center px-3' : '']" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group relative" @click="closeMobile">
      ...
      <NuxtLink v-for="item in otherItems" :key="item.to" :to="item.to" :class="[active === item.active ? 'bg-gradient-to-r from-sky-100 to-blue-100 text-blue-700 shadow-sm' : 'text-gray-600 hover:bg-sky-50 hover:text-sky-700', collapsed ? 'justify-center px-3' : '']" class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group relative" @click="closeMobile">
        <div v-html="item.icon" class="w-5 h-5 flex-shrink-0"></div>
        <span v-if="!collapsed" class="font-medium">{{ item.label }}</span>
        <span v-if="collapsed" class="absolute left-full ml-2 px-3 py-1.5 bg-gray-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 whitespace-nowrap z-50 pointer-events-none transition shadow-lg">{{ item.label }}</span>
      </NuxtLink>
...
const manageItems = [
  { to: '/admin/rooms', active: 'rooms', label: 'Manajemen Kamar', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>' },
  { to: '/admin/room-types', active: 'room-types', label: 'Tipe Kamar', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l5 5a2 2 0 01.586 1.414V19a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z"></path></svg>' },
  { to: '/admin/facilities', active: 'facilities', label: 'Fasilitas', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"></path></svg>' },
  { to: '/admin/users', active: 'users', label: 'Kelola User', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>' },
]

const otherItems = [
  { to: '/admin/reports', active: 'reports', label: 'Laporan Pendapatan', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>' },
